...
public class TeamService extends IntentService {
    ...
    private void getAllTeamsAndPersistThem(ResultReceiver receiver) {
        ...
        for (String league : leagues) {
            try {
                ...
                ArrayList<Team> teamsTemp = SoccerParser.parseTeams(result);
        ...
        for (Team team : teams) {
            ...
            if (null != team) {
                ...
                teamValues.put(TeamEntry._ID, team.getId());
                teamValues.put(TeamEntry.NAME, team.getName());
                teamValues.put(TeamEntry.SHORT_NAME, team.getShortName());
                teamValues.put(TeamEntry.THUMBNAIL, team.getThumbnail());
    ...
    private void insertData(Vector<ContentValues> values) {
        ...
        int inserted_data = getContentResolver().bulkInsert(SoccerContract.TeamEntry.CONTENT_URI, insertData);
